...
class ChatThread extends Thread {
    ...
    public void run() {
        ...
        try {
            while ((msg = in.readLine()) != null) {
                // 귓속말 메시지인지 확인
                if (msg.startsWith("/w ")) {
                    sendPrivateMessage(msg); // 귓속말 메시지 처리
                } else {
                    broadcast(id + " : " + msg); // 일반 메시지 전파
                }
            }
        } catch (IOException e) {
            System.out.println(e);
    ...
    public void sendPrivateMessage(String msg) {
        ...
        if (secondSpaceIndex == -1) return; // 두 번째 공백이 없다는 것도 메시지가 잘못된 거니까...
        ...
        if (pw != null) {
            pw.println(id + "님으로부터 비밀 메시지 : " + message);
        } else {
