...
public class EchoServer2 {
    ...
    public static void main(String[] args) {
        try (ServerSocket serverSocket = new ServerSocket(PORT_NUMBER)) {
            ...
            while (true) {
                Socket clientSocket = serverSocket.accept(); // 클라이언트 연결 수락 System.out.println("새로운 클라이언트가 연결되었습니다: " + clientSocket);
                new Thread(new ClientHandler(clientSocket)).start(); // 클라이언트를 위한 새 스레드 시작
            }
        }
